...
Hiding delete column in movie table if user is not admin - user must be logged in + admin:

Solution:

MoviesTable: separate deleteColumn from the columns array and append it dynamically in the constructor.
```javascript
